...
protected:
    static void
    merge(std::vector<GrainOfSand> &grains, uint64_t begin, uint64_t mid, uint64_t end) {
        if (mid < end) {
    ...
    static void
    arrangeSandSequential(std::vector<GrainOfSand> &grains, uint64_t begin, uint16_t end) {
    ...
    static Crystal
    selectBestSequential(std::vector<Crystal> &crystals, uint64_t begin, uint64_t end) {
        uint64_t best_crystal = 0;
        
        for (uint64_t i = begin; i < end; i++) {
            if (crystals[best_crystal] < crystals[i])
                best_crystal = i;
        }
    ...
    dividesInEqualPart(uint64_t size, uint64_t workers) {
        ...
        for (uint64_t i = 1; i <= eggs_number; i++) {
            for (uint64_t j = 0; j <= bag_capacity; j++) {
                if (eggs[i - 1].getSize() > j) {
                    ...
                    for_dynamic[i][j] = std::max(for_dynamic[i - 1][j],
...
private:
    ...
    public:
        CyclicBarrier(uint64_t resistance) : thread_number(resistance), index{0},
    ...
    public:
        void await() {
            ...
            if (current_resistance[current_index] < thread_number) {
                cv.wait(lock, [&] { return current_resistance[current_index] >= thread_number; });
            } else {
                index ^= 1;
    ...
    static void packEggsThread(CyclicBarrier &barier,
                               uint64_t begin_bag_capacity,
                               uint64_t end_bag_capacity,
                               uint64_t **for_dynamic,
                               uint64_t eggs_number,
                               std::vector<Egg> eggs,
    ...
    getMergeTasks(std::vector<std::pair<uint64_t, uint64_t>> &equal_parts) {
        std::vector<std::tuple<uint64_t, uint64_t, uint64_t>> result;
        for (uint64_t i = 0; i < equal_parts.size(); i += 2) {
            if (i != equal_parts.size() - 1) {
                result.push_back(std::make_tuple(equal_parts[i].first, equal_parts[i].second,
    ...
    std::vector<std::tuple<uint64_t, uint64_t, uint64_t>>
    joinMergeTask(const std::vector<std::tuple<uint64_t, uint64_t, uint64_t>> &merge_task) {
        std::vector<std::tuple<uint64_t, uint64_t, uint64_t>> result;
        
        if (merge_task.size() == 1)
            return result;
...
public:
    ...
    virtual void arrangeSand(std::vector<GrainOfSand> &grains) {
        ...
        for (uint64_t i = 0; i < equal_parts.size(); i++) {
            sorting_task[i] = councilOfShamans.enqueue(TeamAdventure::arrangeSandSequential,
                                                       std::ref(grains),
                                                       equal_parts[i].first,
                                                       equal_parts[i].second);
        }
        ...
        while (merge_task_parts.size() > 0) {
            std::future<void> *merge_task = new std::future<void>[merge_task_parts.size()];
            for (uint64_t i = 0; i < merge_task_parts.size(); ++i) {
                merge_task[i] = councilOfShamans.enqueue(TeamAdventure::merge,
                                                         std::ref(grains),
                                                         std::get<0>(merge_task_parts[i]),
                                                         std::get<1>(merge_task_parts[i]),
    ...
    virtual uint64_t packEggs(std::vector<Egg> eggs, BottomlessBag &bag) {
        ...
        for (uint64_t i = 0; i < equal_parts.size(); i++) {
            knapsack_tasks[i] = councilOfShamans.enqueue(TeamAdventure::packEggsThread,
                                                         std::ref(barrier),
                                                         equal_parts[i].first,
                                                         equal_parts[i].second,
                                                         for_dynamic,
    ...
    virtual Crystal selectBestCrystal(std::vector<Crystal> &crystals) {
        ...
        for (uint64_t i = 1; i < numberOfShamans; i++) {
            ...
            if (best < best_in_partition)
                best = best_in_partition;
        }
        return best;
    }
